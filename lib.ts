...
async function handleDmEvent(ev: Record<string, unknown>, opts: GateOptions): Promise<GateResult> {
  ...
  if (access.dmPolicy === 'allowlist' || access.dmPolicy === 'disabled') {
    ...
    // forwarding to MCP. Still a drop — no Claude tokens consumed. The
    // upstream `dropReason` (ccsc-apj.2) is preserved for audit either
    // way so the journal shows *why* Claude stayed silent.
    const cannedReply = access.deniedDmReply?.trim()
    if (cannedReply) return { action: 'drop', cannedReply, dropReason: 'dm.policy_closed' }
